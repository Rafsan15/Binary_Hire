...
#             r'(?P<fmonth>(?:\w+|\w{3}|[01]?\d)[ ,/-]*\d{2,4})\s*(?:[-–]|\sto\s)\s*(?P<smonth>(?:\w+|\w{3}|[01]?\d)[ ,/-]*\d{2,4}|present|current)',
#             line,
#             re.I
#         )
#         if experience:
...
def get_total_experience(experience_list):
    """
    Wrapper function to extract total months of experience from a list of experience text.
    ...
    for line in experience_list:
        matches = re.finditer(
            r'\b(?:(?:\d{1,2}[ ,/-]*)?(?:\w+|\w{3}|[01]?\d)[ ,/-]*\d{2,4}|present|current)\b',
            line,
            re.I
        )
        ...
        #print(dates)
        if len(dates) >= 2:
            exp_.append({'fmonth': dates[0], 'smonth': dates[1]})
    #print(exp_)
    total_exp = sum(
        [get_number_of_months_from_dates(i['fmonth'], i['smonth']) for i in exp_]
    )
...
def get_number_of_months_from_dates(start_date, end_date):
    ...
    """
    if start_date.lower() in ['present', 'current']:
