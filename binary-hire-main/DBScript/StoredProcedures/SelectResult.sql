--region PROCEDURE [dbo].[SelectResult] @JobPostId=7, @OrganizationId=1
IF OBJECT_ID('[dbo].[SelectResult]') IS NOT NULL 
    BEGIN 
        DROP PROC [dbo].[SelectResult] 
    END 
GO 
     
CREATE PROC [dbo].[SelectResult] 
( 
	@JobPostId INT,
	@Page INT = 1,
	@PageSize INT = 10,
...
BEGIN
	...
	SET @Query = N'
				...
				LEFT JOIN [dbo].[Screening] AS SC ON SC.Id = R.ScreeningId
				LEFT JOIN [dbo].[JobPost] AS JP ON JP.Id = SC.JobPostId
				WHERE JP.OrganizationId=@OrganizationId AND JP.Id = @JobPostId' ;

	SET @ParmDefinition = '@FirstRec int, @LastRec int, @OrganizationId int, @JobPostId int';
	...
	EXEC sp_executesql @Query, @ParmDefinition, @FirstRec=@FirstRec, @LastRec= @LastRec, @OrganizationId=@OrganizationId, @JobPostId=@JobPostId;
