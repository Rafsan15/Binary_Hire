...
const Finished = ({ job, handleFavoriteToggle }) => {
  ...
  return (
    <Link className="flex" to={`/result/${job.id}`}>
      <div className="relative mb-4 text-gray-700 bg-white shadow-md border bg-clip-border rounded-xl w-full hover:bg-blue-100 transition-colors duration-300">
        <Tooltip
        ...
        <button
          ...
          type="button"
          className="absolute top-2 right-2 mt-4 mr-2 bg-white rounded-full p-1 transition-colors duration-300 hover:bg-blue-100"
        >
          {job.isFav ? (
        ...
        <div className="p-6 pt-0 flex items-center justify-between">
          <div className="block w-[90px] text-center font-sans text-base antialiased font-semibold leading-relaxed text-blue-600 bg-blue-100 p-1 rounded-md">
            {job.statusName}
          </div>
        </div>
      </div>
