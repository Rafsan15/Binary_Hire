...
public class BackgroundTaskService : IBackgroundTaskService
{
    ...
    public async Task StartBackgroundTask()
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                try
                {
                    ExtractFile.sharedFolderLink = queuedProcess.Data.LocationPath;
                    fileLinks = ExtractFile.Extract();
                }
                catch(Exception e)
                {
                    isRunning = false;
                ...
                var files = GetFilesInDirectory(ExtractFile.localFolderPath, allowedExtensions);
                var HighExp = 0.0;
                ...
                foreach (var v in files)
                {
                    try
                    {
                        var response = BH.ML.ResultProcessing.ProccessCV(v);
    ...
    private List<EvaluationModel> getScore(List<EvaluationModel> resumeResult, WorkflowResponseModel? workflowData, JobPostResponse? jobReqData, double highExp)
    {
        ...
        foreach (var v in resumeResult)
        {
            var score = 0.0;
            if(weightPercentage.ContainsKey(SettingConstants.WORKFLOW_EXPERIENCE) && v.Experience>=jobReqData.MinExperience)
            {
                score += (weightPercentage[SettingConstants.WORKFLOW_EXPERIENCE] / (highExp*100)) * (v.Experience * 100);
            }
            if(weightPercentage.ContainsKey(SettingConstants.WORKFLOW_SKILL))
            {
                if (v.Skills == null)
                    v.Skills = new string[0];
            ...
            {
                if (v.Education == null)
                    v.Education = new string[0];
                score += (weightPercentage[SettingConstants.WORKFLOW_EDUCATION] * CalculateAverageSimilarityPercentage(jobReqData.Education,v.Education)) / 100;
            }

            v.Match = Math.Round(score, 2);
