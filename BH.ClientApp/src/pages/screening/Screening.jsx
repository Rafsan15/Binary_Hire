...
const Screening = () => {
  ...
  //   setCurrentReqState("hello");
  //   console.log("37 line")
  //   console.log(currentReqState)
  // },[])
  ...
  useEffect(() => {
    const setElements = async (workfId) => {
      try {
        ...
        const response = await axios.post(`${BASE_URL}WorkFlow/get-by-id`, requestBody, {
          headers: {
    ...
    const getInitWorkflow = async () => {
      try {
        ...
        const response = await axios.post(`${BASE_URL}Result/get-workflow-by-job-id`, requestBody, {
          headers: {
            Authorization: `Bearer ${authToken}`,
            'Content-Type': 'application/json',
            userId,
    ...
    const getInitStatus = async () => {
      try {
        ...
        const response = await axios.post(`${BASE_URL}JobPost/get-current-request-status-by-id`, requestBody, {
          headers: {
            Authorization: `Bearer ${authToken}`,
            'Content-Type': 'application/json',
            userId,
        ...
        else if(response.data.data && (response.data.data == 'Queued' || response.data.data == 'In Progress')){
  ...
  useEffect(() => {
    if(currentReqState == 'In Progress'){
      //getCurrentProgress();
      setVisibleLoaders([Math.floor(currentProgress/(100 / droppedElements.length))])
      ...
      <div className='flex-1 bg-[#F1F5F9] '>
        <div className='p-20'>
          {isInputVisible ? (
            ...
            <div className='mb-12 '>
              <h2 className='block text-lg font-bold mb-2'>
                Folder Link of Cloud<span className='text-red-500'>*</span>
              </h2>
              <input
              type='text'
              id='CV_link'
          ...
          <div className='mx-auto p-10 max-w-2xl border bg-white border-gray-300 rounded-md shadow-md'>
            <span>
                Current Status : {currentReqState?currentReqState : 'Not Started'}
            </span>
            <ProgressBar
              completed={progress}
              bgColor='#5674FC'
